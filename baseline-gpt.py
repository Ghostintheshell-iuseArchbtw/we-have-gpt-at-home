...
server_url = 'http://<IPADDRESSOFLLMSTUDIO>/v1/chat/completions'
...
def make_inference_request(prompt):
    ...
    payload = {
        "messages": [
            {"role": "system", "content": "You are my AI Assistant."},  #########ADD YOUR PROMPT HERE IF YOU WANT TO CHANGE THE SYSTEM PROMPT 
            {"role": "user", "content": prompt}
        ],
        "temperature": 0.7,
        "max_tokens": 150,  # Limit max tokens for response
    ...
    try:
        with requests.post(server_url, json=payload, headers={"Content-Type": "application/json"}, stream=True) as response:
            ...
            for line in response.iter_lines():
                if line:
                    line_str = line.decode('utf-8').strip()
                    if line_str.staimport requests
import json
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    try:
                        ...
                        print(f"Non-JSON responsimport requests
import json
...
def main():
    ...
    while True:
        prompt = input("You: ")import requests
import json
